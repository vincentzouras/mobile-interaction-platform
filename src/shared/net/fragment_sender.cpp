...
FragmentSender::FragmentSender(UDPTransmitter& transmitter) : udp_tx(transmitter) {}

void FragmentSender::send_payload(const std::vector<uint8_t>& payload) {
    if (payload.empty()) return;

    // Calculate fragment math
    uint16_t total_fragments = (payload.size() / net::MAX_FRAGMENT_DATA_SIZE) +
                               ((payload.size() % net::MAX_FRAGMENT_DATA_SIZE) != 0 ? 1 : 0);

    // Send fragments
    for (uint16_t i = 0; i < total_fragments; ++i) {
        size_t offset = i * net::MAX_FRAGMENT_DATA_SIZE;
        size_t bytes_to_send =
            std::min((size_t)net::MAX_FRAGMENT_DATA_SIZE, payload.size() - offset);

        // Prepare the packet buffer
        packet_buffer.resize(sizeof(FragmentHeader) + bytes_to_send);
        ...
        FragmentHeader network_header = host_header;
        ...
        std::memcpy(packet_buffer.data(), &network_header, sizeof(FragmentHeader));

        // Copy payload data into packet right after the header
        std::memcpy(packet_buffer.data() + sizeof(FragmentHeader), payload.data() + offset,
                    bytes_to_send);
